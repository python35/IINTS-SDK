...
def analyze_study_directory(
    ...
    *,
    ...
    tir_values = [run.metrics.get("tir_70_180") for run in runs]
    intervention_values = [run.metrics.get("supervisor_interventions") for run in runs]
    mean_glucose_values = [run.metrics.get("mean_glucose") for run in runs]
    cv_values = [run.metrics.get("cv") for run in runs]
    below_70_values = [run.metrics.get("tir_below_70") for run in runs]
    below_54_values = [run.metrics.get("tir_below_54") for run in runs]
    above_180_values = [run.metrics.get("tir_above_180") for run in runs]
    above_250_values = [run.metrics.get("tir_above_250") for run in runs]
    gmi_values = [run.metrics.get("gmi") for run in runs]
    ...
    aggregate = {
        "mean_tir_70_180": _mean(tir_values),
        "mean_tir_below_70": _mean(below_70_values),
        "mean_tir_below_54": _mean(below_54_values),
        "mean_tir_above_180": _mean(above_180_values),
        "mean_tir_above_250": _mean(above_250_values),
        "mean_supervisor_interventions": _mean(intervention_values),
        "mean_glucose": _mean(mean_glucose_values),
        "mean_cv": _mean(cv_values),
        "mean_gmi": _mean(gmi_values),
        "run_count": len(runs),
    }

    aggregate_stats = {
        "tir_70_180": _stats(tir_values),
        "tir_below_70": _stats(below_70_values),
        "tir_below_54": _stats(below_54_values),
        "tir_above_180": _stats(above_180_values),
        "tir_above_250": _stats(above_250_values),
        "supervisor_interventions": _stats(intervention_values),
        "mean_glucose": _stats(mean_glucose_values),
        "cv": _stats(cv_values),
        "gmi": _stats(gmi_values),
    }
