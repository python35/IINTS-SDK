...
ENTRYPOINT = REPO_ROOT / "src" / "iints_desktop" / "qt_app.py"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the IINTS-AF PySide/Qt desktop app with PyInstaller.")
    ...
    if not ENTRYPOINT.exists():
        raise SystemExit(f"Qt desktop entrypoint not found: {ENTRYPOINT}")

    require_module("PyInstaller", 'python -m pip install -U -e ".[desktop-qt]"')
