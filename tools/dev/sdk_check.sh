...
Environment:
  IINTS_RELEASE_VERSION=1.5.4  Required for release mode.
USAGE
}
...
case "$MODE" in
  ...
  release)
    if [[ -z "${IINTS_RELEASE_VERSION:-}" ]]; then
      echo "Set IINTS_RELEASE_VERSION first, for example: IINTS_RELEASE_VERSION=1.5.4 tools/dev/sdk_check.sh release" >&2
      exit 2
    fi
    run_full
